...
Usage:
    ...
    python train.py
    python train.py --target-column treatment
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--target-column",
        type=str,
        default=config.DEFAULT_TARGET_COLUMN,
...
def main() -> None:
    ...
    logger.info(
        "PyTorch        : %s",
        "enabled" if run_pytorch else "disabled",
    )
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info(
            "Dataset loaded: %d rows x %d columns.",
            df.shape[0],
            df.shape[1],
        )
    ...
    try:
        ...
        logger.error(
            "Sklearn training failed: %s",
            exc,
        )
        sys.exit(1)

    # ------------------------------------------------------------------ #
    # 3. Evaluate Scikit-Learn Models
    # ------------------------------------------------------------------ #
    ...
    for name, result in sk_results.items():
        ...
        # IMPORTANT:
        # Keep X_test as a pandas DataFrame.
        #
        # The models were fitted using feature names.
        # Using X_test.values removes those names and causes:
        #
        # "X does not have valid feature names..."
        #
        # Therefore DO NOT use X_test.values here.

        y_pred = result.model_instance.predict(
            X_test
        )
        ...
        if hasattr(
            ...
            try:
                y_prob = result.model_instance.predict_proba(
                    X_test
                )

            except Exception as exc:
                logger.warning(
                    "Could not generate probabilities for "
                    "%s: %s",
                    name,
                    exc,
                )
    ...
    if run_pytorch:
        ...
        try:
            ...
            pytorch_trainer.train_model(
                X_train=X_train.values.astype(
                    np.float32
                ),
                y_train=feature_engineer.encode_labels(
                    y_train
                ),
                X_val=X_val.values.astype(
                    np.float32
                ),
                y_val=feature_engineer.encode_labels(
                    y_val
                ),
            ...
            # ---------------------------------------------------------- #
            # Basic PyTorch Evaluation
            # ---------------------------------------------------------- #

            pt_eval = pytorch_trainer.evaluate(
                X_test=X_test.values.astype(
                    np.float32
                ),
                y_test=feature_engineer.encode_labels(
                    y_test
                ),
            )
            ...
            X_test_pt = X_test.values.astype(
                np.float32
            )

            y_test_pt = feature_engineer.encode_labels(
                y_test
            )
    ...
    # Determine which Scikit-Learn model is best.

    best_sk_model_instance = (
        sk_results[
            best_sk_name
        ].model_instance
    )
    ...
    saved_paths = saver.save_all(
        ...
        pytorch_model=(
            pytorch_trainer.model
            if pytorch_trainer
            else None
        ),

        label_encoder=(
            feature_engineer.get_label_encoder()
        ),
